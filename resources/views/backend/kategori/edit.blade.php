...
<x-card title="kategori">
    ...
    <form action="{{ route('manage.kategori.update', $kategori) }}" method="post" class="row" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama" id="nama" class="form-control" value="{{ $kategori->nama }}" placeholder="" aria-describedby="helpId" required>
            </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select class="form-control form-control-sm select2" name="parent_id" id="parent_id">
                    <option value="">-- Pilih Katgeori Parent --</option>
                    @foreach ($parents as $item)
                      <option value="{{ $item->id }}" {{ $kategori->parent_id == $item->id ? 'selected' : ''}}>{{ $item->nama }}</option>                  
                    @endforeach
                </select>
            </div>
        </div>
